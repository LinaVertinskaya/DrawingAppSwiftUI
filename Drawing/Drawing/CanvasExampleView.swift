...
struct CanvasExampleView: View {
    var body: some View {
        Canvas { context, size in
            ...
            context.stroke(Path(roundedRect: CGRect(x: 10, y: 10, width: 100, height: 100), cornerRadius: 20),
            ...
            context.drawLayer { subcontext in
                ...
                subcontext.fill(Path(ellipseIn: CGRect(x: 100, y: 0, width: 200, height: 150)), with: .color(.green))
            }

            context.stroke(Path(ellipseIn: CGRect(x: 100, y: 0, width: 200, height: 150)), with: .color(.green))
